...
def cifar10_tutorial(train_start=0, train_end=60000, test_start=0,
  ...
  if True:
      def custom_training_data(x, y):
        from util import load_pkl
        import branch_net as bn

        print("Training without samples that are consistantly misclassified: ")
        # Load stats from training history
        stats_test, stats_train = load_pkl("stats_25_train_test.pkl", data_dir='./')
        correctcount_train, correctcount_test = stats_train[25], stats_test[25]

        label_train = np.argmax(y, axis=1)
        (x_train_easy, y_train_easy), (x_train_hard, y_train_hard) = \
            bn.binary_split(label_train, correctcount_train, x, threshold=1)

        x_, y_ = x_train_easy, y_train_easy
        return x_, y_

      x_train, y_train = custom_training_data(data.x_train, data.y_train)
